...
worauf an dieser Stelle allerdings nicht weiter eingegangen werden soll. In
...
Leitungs- bzw. Valenzbandes (siehe \ref{subfig:pn-halbleiter_a}).
\begin{figure}
 	\centering
	\subfigure[]{
		\label{subfig:pn-halbleiter_a}
    	\includegraphics[width=7cm]{gfx/pn-halbleiter_a}
  	}
	\subfigure[]{
		\label{subfig:pn-halbleiter_b}
    	\includegraphics[width=7cm]{gfx/pn-halbleiter_b}
  	}
  	\subfigure[]{
		\label{subfig:pn-halbleiter_c}
    	\includegraphics[width=7cm]{gfx/pn-halbleiter_c}
  	}
	\subfigure[]{
		\label{subfig:pn-halbleiter_d}
    	\includegraphics[width=7cm]{gfx/pn-halbleiter_d}
  	}
	\caption{Bandstruktur eines p-n-Halbleiters: a)
...
\begin{figure}[h]
	\centering
	\includegraphics[width=15cm]{gfx/heterostrukturen}
	\caption{Verschiedene Heterostrukturen;
...
Schichtgrenzen realisiert werden. Dabei ist der dieser allerdings nur in eine
...
\begin{figure}[h]
	...
	\subfigure[]{
    	...
    	\includegraphics[width=7cm]{gfx/transversale_eingrenzung_diodenlaser_a}
  	}
	\subfigure[]{
		\label{subfig:transversale_eingrenzung_diodenlaser_b}
    	\includegraphics[width=7cm]{gfx/transversale_eingrenzung_diodenlaser_b}
  	}
	\caption{Gewinngeführter (a) und indexgeführter (b) Aufbau einer
	kantenemittierenden Laserdiode. Der aktive Bereich
...
\begin{figure}[h]
 	\centering
	\subfigure[]{
		\label{subfig:TEM-moden_a}
    	\includegraphics[width=7cm]{gfx/TEM-moden_a}
  	}
	\subfigure[]{
		\label{subfig:TEM-moden_b}
    	\includegraphics[width=7cm]{gfx/TEM-moden_b}
  	}
  	\subfigure[]{
		\label{subfig:TEM-moden_c}
    	\includegraphics[width=7cm]{gfx/TEM-moden_c}
  	}
	\caption{Amplituden der fundamentalen Mode (a), der ersten (b)
	und der zweiten (c) Harmonischen.}\label{fig:TEM-moden}
\end{figure}
...
einer kleiner effektiveren Energielücke bzw. kleineren Frequenz. Gleichzeitig
...
\begin{figure}[h]
 	\centering
	\subfigure[]{
		\label{subfig:externe_resonatoren_littrow}
    	\includegraphics[width=15cm]{gfx/externe_resonatoren_littrow}
  	}
	\subfigure[]{
		\label{subfig:externe_resonatoren_littmann}
    	\includegraphics[width=15cm]{gfx/externe_resonatoren_littmann}
  	}
	\caption{Externe Resonatoren im Littow- (a) und
	im Littmann-Design (b)}\label{fig:externe_resonatoren}
\end{figure}
Abbildung \ref{fig:externe_resonatoren_littrow} zeigt das sog.
\textit{Littrow-Design}, bei dem durch Verkippen des Gitters die Wellenlänge
\begin{equation}\label{eq:FSR_TEM-moden}
	\lambda=2g\sin{(\alpha)}
\end{equation}
selektiert werden kann. Durch Verdrehen eines Auskoppelspiegels um dieselbe
Achse kann die nullte Ordnung ohne Winkeländerung ausgekoppelt werden. Der
...
\ref{fig:externe_resonatoren_littmann}). Dabei wird die erste Ordnung des
Gitters auf einen Spiegel und in dieselbe Richtung wieder zurück in die Diode
gekoppelt. Die Wellenlänge
\begin{equation}\label{eq:FSR_TEM-moden}
...
Die anschwingende Mode kombiniert sich nun aus internem und externem Resonator
und dem Verstärkungsprofil wie in Abb. \ref{fig:moden_frequenzselektion}
dargestellt.
\begin{figure}[h]
	\centering
	\footnotesize
	\input{plt/moden_frequenzselektion}
	\caption{Verstärkungsprofil, internes und
	externes Modenprofil und anschwingende Mode des
	Lasers}\label{fig:moden_frequenzselektion}
\end{figure}
Nur die Mode mit der größten Verstärkung schwingt an, alle anderen
werden unterdrückt. Damit die Frequenz über einen möglichst großen Bereich
verstimmt werden kann, muss das Modenprofil des internen und externen Resonators
gleichzeitig verstimmt werden. Strom und Gitterwinkel müssen als simultan
verstimmt werden, wobei das Gitter kann mit Hilfe eines Piezos verdreht werden
kann. Dieses Verfahren nennt man \textit{Feed Forward}.\par
Alternativ ist es auch möglich ein \textit{AR-coating (Antireflex-coating)} für
die Diode zu verwenden, und somit den Laser ohne internen Resonator betreiben.
Dies bringt den Vorteil, dass nur noch der Gitterwinkel verstellt werden muss,
der Strom und somit auch weitgehenst die Leistung konstant bleiben. Nachteil
ist, dass die Diode alleine nicht mehr als Laserdiode verwednet werden kann, da
